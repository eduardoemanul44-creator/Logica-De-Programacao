...
Sistema de notificação automática de notas via Google Apps Script. Lê os dados de uma planilha do Google Sheets e envia um e-mail personalizado pra cada aluno com a nota e a situação (Aprovado / Recuperação / Reprovado).

> **Atenção:** este código roda dentro do editor do Google Apps Script, vinculado a uma planilha do Google Sheets — não é um script Node.js comum. Ele está aqui como referência de portfólio; pra funcionar de verdade, precisa ser colado no editor do Apps Script de uma planilha (veja "Como usar" abaixo).
...
**`enviarEmails()`**
Percorre todas as linhas da planilha de uma vez e envia um e-mail pra cada aluno. Útil pra disparar tudo em lote, manualmente.

**`aoEditar(e)`**
Roda automaticamente sempre que alguém edita a planilha. Verifica se a linha editada está completa e se o e-mail ainda não foi enviado (marca "✅ Enviado" na coluna de confirmação) antes de disparar — evita duplicidade.
